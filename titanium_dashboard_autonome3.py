"""
Titanium Dashboard — AUTONOME + Vision locale Ollama (STABILITÉ / WS TEXTE PUR)
- Bridge conservé mais désactivé par défaut (ENABLE_BRIDGE=0)
- Lifespan context manager (FastAPI) [Source](https://www.genspark.ai/api/files/s/OVow31JK)
- Compteur de clients WS + métriques /api/metrics
- Scoring 24/7 : suppression du critère SESSION 8-18 [Source](https://www.genspark.ai/api/files/s/OVow31JK)
...
- Vision locale Ollama: /api/chat + images:[base64] [Source](https://docs.ollama.com/capabilities/vision)
- LLaVA prioritaire + Qwen2.5-VL 3B fallback [Source](https://ollama.com/library/qwen2.5vl/tags)
"""
...
# Score minimum pour déclencher une alerte (sur 7). Alertes à 4/7, 5/7, 6/7, 7/7.
TELEGRAM_SCORE_THRESHOLD = float(os.getenv("TELEGRAM_SCORE_THRESHOLD", "4"))
# Labels de qualité par palier de score /7
TELEGRAM_SCORE_LABELS: Dict[int, str] = {4: "⚠️ Surveillance", 5: "✅ Bon setup", 6: "🔥 Setup fort", 7: "🚀 Setup optimal"}
...
STRICT_MIN_TRADES = int(os.getenv("STRICT_MIN_TRADES", "8"))
STRICT_SHARPE_FLOOR = float(os.getenv("STRICT_SHARPE_FLOOR", "0.5"))  # 0.5 = seuil réaliste sur 30j
STRICT_SUBPERIOD_DAYS = int(os.getenv("STRICT_SUBPERIOD_DAYS", "30"))  # 30j/période → 6 périodes sur 180j
STRICT_ROBUST_MIN_PERIODS = int(os.getenv("STRICT_ROBUST_MIN_PERIODS", "2"))  # 2 périodes min (assoupli)
STRICT_TOPK_ZONES = int(os.getenv("STRICT_TOPK_ZONES", "3"))  # nb de zones candidates
STRICT_ZONE_MIN_DIST = int(os.getenv("STRICT_ZONE_MIN_DIST", "10"))  # distance min entre 2 zones

# Mode PRO: recalibrage STRICT dans un process séparé (évite toute latence live)
STRICT_OFFLOAD_PROCESS = os.getenv("STRICT_OFFLOAD_PROCESS", "1").strip().lower() in ("1","true","yes","on")
...
def record_signal_outcome(sym: str, entry: float, sl: float, tp1: float, current_price: float,
    ...
    """Enregistre le résultat d'un signal passé (appelé depuis scan_loop à chaque cycle)."""
    ...
    for rec in hist:
        if rec.get("outcome") == "pending":
            ...
            if "ACHAT" in rec.get("side", ""):
                if current_price >= tp2_r:
                    rec["outcome"] = "tp2_hit"
                elif current_price >= tp1_r:
                    rec["outcome"] = "tp1_hit"
                elif current_price <= sl_r:
                    rec["outcome"] = "sl_hit"
            else:
                if current_price <= tp2_r:
                    rec["outcome"] = "tp2_hit"
...
def add_signal_to_history(sym: str, score: int, side: str, confs: List[str],
    ...
    """Ajoute un nouveau signal en statut pending."""
...
def _compute_learning_report(sym: str) -> dict:
    """Calcule un rapport de performance par critère pour un symbole."""
    ...
    total = len(hist)
    wins  = [r for r in hist if r.get("outcome") in ("tp1_hit", "tp2_hit")]
    losses = [r for r in hist if r.get("outcome") == "sl_hit"]
    win_rate = len(wins) / total if total else 0.0

    # Performance par critère : taux de victoire des signaux qui contenaient ce critère
    ...
    for crit in SCORE_CRITERIA:
        ...
        if not crit_signals:
            ...
            continue
        crit_wins = [r for r in crit_signals if r.get("outcome") in ("tp1_hit", "tp2_hit")]
        crit_wr = len(crit_wins) / len(crit_signals)
        # Poids suggéré : ratio par rapport au win_rate global (+ si meilleur, - si pire)
...
async def learning_report_loop(session: aiohttp.ClientSession) -> None:
    ...
    await asyncio.sleep(60)  # attendre 1 min après démarrage
    while True:
        try:
            now = datetime.now(timezone.utc).timestamp()
...
STRICT_JOB_TIMEOUT_MIN = int(os.getenv("STRICT_JOB_TIMEOUT_MIN", "45"))  # max 45 min par job
...
VISION_MODEL_PRIMARY = os.getenv("VISION_MODEL_PRIMARY", "llava")
VISION_MODEL_FALLBACK = os.getenv("VISION_MODEL_FALLBACK", "qwen2.5vl:3b-q4_K_M")  # [Source](https://ollama.com/library/qwen2.5vl/tags)

VISION_NUM_CTX = int(os.getenv("VISION_NUM_CTX", "2048"))
VISION_KEEP_ALIVE = os.getenv("VISION_KEEP_ALIVE", "300s")  # 300s = 5min en RAM
...
if VISION_KEEP_ALIVE and VISION_KEEP_ALIVE.strip() not in ("0", "-1"):
    ...
    if _kav.lstrip("-").isdigit() and int(_kav) > 0:
        VISION_KEEP_ALIVE = _kav + "s"  # ajouter l'unité manquante automatiquement
VISION_TIMEOUT_PRIMARY = int(os.getenv("VISION_TIMEOUT_PRIMARY", "180"))  # 3min pour llava sur CPU
VISION_TIMEOUT_FALLBACK = int(os.getenv("VISION_TIMEOUT_FALLBACK", "60"))
...
async def ollama_vision_analyze(session: aiohttp.ClientSession, image_b64: str, symbol: str, timeframe: str, price: float, side_hint: Optional[str], algo_context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    img_note  = "Analyse le graphique fourni en image." if has_image else "Aucune image fournie — base ton analyse sur le contexte algorithmique ci-dessous uniquement."
    score_max = algo_context.get("score_max", 7)
...
- TF: {timeframe}
- Prix actuel: {price}
- Direction hint: {side_hint or "N/A"}
- Score SMC: {algo_context.get("score", "N/A")}/{score_max}
- Confluences: {algo_context.get("confs", [])}
- Structure H2: {algo_context.get("struct_h2", "N/A")} | H1: {algo_context.get("struct_h1", "N/A")}
- OB status 30m: {algo_context.get("ob_status_30m", "N/A")} | qualité: {algo_context.get("ob_quality_30m", "N/A")}
- Double confirm FVG⊂OB: {algo_context.get("ob_fvg_15m_ok", False)}
- RSI 5m: {algo_context.get("rsi_5m", "N/A")}
- Range 30m: {algo_context.get("range_30m", False)}
- Données complètes: {json.dumps(algo_context, ensure_ascii=False)[:600]}
UTC: {datetime.now(timezone.utc).strftime("%H:%M")}

Réponds UNIQUEMENT avec le JSON brut — aucun texte avant ou après.
"""

    async def _try(model_name: str, timeout_s: int) -> Dict[str, Any]:
        raw = await _ollama_chat(session, model_name, user_prompt, image_b64, timeout_s=timeout_s)
    ...
    try:
        result = await _try(VISION_MODEL_PRIMARY, VISION_TIMEOUT_PRIMARY)
        ...
        try:
            result = await _try(VISION_MODEL_FALLBACK, VISION_TIMEOUT_FALLBACK)
            ...
            err_msg = str(e2)[:200]
            if "timeout" in err_msg.lower() or elapsed2 > VISION_TIMEOUT_FALLBACK - 5:
                err_msg = (
                    f"Timeout ({elapsed2}s) — LLaVA est lent sur CPU ({elapsed1:.0f}s). "
                    f"Augmente VISION_TIMEOUT_PRIMARY (actuel={VISION_TIMEOUT_PRIMARY}s) dans .env, "
                    f"ou utilise qwen2.5vl:3b-q4_K_M comme modèle primaire."
                )
            result = _empty_vision(f"Vision error: {err_msg}")
...
def _detect_market_structure(df: pd.DataFrame, n: int = 20) -> str:
    ...
    # Cherche HH/HL (bullish) ou LH/LL (bearish) sur les pivots simplifiés
    pivot_highs = [highs[i] for i in range(1, len(highs)-1) if highs[i] > highs[i-1] and highs[i] > highs[i+1]]
    pivot_lows  = [lows[i]  for i in range(1, len(lows)-1)  if lows[i]  < lows[i-1]  and lows[i]  < lows[i+1]]
...
def score_setup(
    ...
    """Scoring multi-timeframe SMC — score /7.

    Critères (7 points max) :
      ...
      4. Bougie de confirmation 15m — Rejet / engulf / pin bar        [+1]
      5. Signal Trix Strict 5m — Entrée optimale (ou RSI 5m fallback) [+1]

    Poids adaptatifs (scoring_w) : chaque critère peut être pondéré
    différemment selon l'historique d'apprentissage (Module 4).
    Le score pondéré est arrondi à l'entier le plus proche, max 7.
    """
    confs: List[str] = []
    score_raw = 0
    w = scoring_w or {}
    ...
    # Critère 2b : BONUS H2+H1 alignés → +1 point effectif
    align_bonus = False
    ...
    if df_m30 is not None and len(df_m30) >= 5:
        ob_fvg_ok, ob_status_30m, ob_quality_30m = _has_ob_or_fvg_alignment(df_m30, side, price_now, lookback=60)
        if ob_fvg_ok:
            confs.append("OB/FVG(30m)[{}]".format(ob_status_30m))
    if not ob_fvg_ok and df30 is not None and len(df30) >= 5:
        ob_fvg_ok, ob_status_30m, ob_quality_30m = _has_ob_or_fvg_alignment(df30, side, price_now, lookback=120)
        if ob_fvg_ok:
    ...
    # Critère 3b : BONUS double confirmation OB/FVG 15m ──────────────────────
    ob_fvg_15m_ok = False
    ob_quality_15m = 0.0
    if ob_fvg_ok and df_m15 is not None and len(df_m15) >= 5:
        ob_fvg_15m_ok, ob_status_15m, ob_quality_15m = _has_ob_or_fvg_alignment(df_m15, side, price_now, lookback=40)
        if ob_fvg_15m_ok:
            score_raw += float(w.get("OB_FVG_15M_CONFIRM", 1.0)) * max(0.5, ob_quality_15m)
            confs.append("OB/FVG(15m)[{}]".format(ob_status_15m))

    # ── Critère 4 : Bougie de confirmation 15m ───────────────────────────────
    rejection_ok = False
    ...
    if not entry_signal and df_m5 is not None and len(df_m5) >= 20:
        ...
        if poi_ok:
            ...
            if "ACHAT" in side and rsi5_val <= RSI_ENTRY_LONG and rsi10_val <= RSI_ENTRY_LONG + 5:
                entry_signal = True
                confs.append("RSI-OK(5m+10m)")
            elif "VENTE" in side and rsi5_val >= RSI_ENTRY_SHORT and rsi10_val >= RSI_ENTRY_SHORT - 5:
                entry_signal = True
                confs.append("RSI-OK(5m+10m)")
    ...
    # ── Finalisation score ────────────────────────────────────────────────────
    score = min(7, int(round(score_raw)))
...
async def ws_binance(session: aiohttp.ClientSession):
    """Flux live robuste — v4 : latence quasi-nulle via debounce 100ms.

    Optimisations v4 :
      - _resample_lock par symbole : empêche les appels concurrents à resample_and_push.
      - Debounce 100ms : resample déclenché dès qu'un nouveau trade arrive sur
        une NOUVELLE seconde, sans attendre la seconde suivante.
      - _append_bar extrait de la boucle (pas de redéfinition à chaque trade).
      - Heartbeat réduit à 15s (détection de coupure plus rapide).
    """
    streams = "/".join([f"{ws_sym(s)}@trade" for s in SYMBOLS])
    ...
    while True:
        ...
        for base in WS_BASES:
            ...
            try:
                async with session.ws_connect(url, heartbeat=15, timeout=20) as ws:
                    connected_any = True
                    logger.info("Binance WS connecté (trade) via %s", base)
                    async for msg in ws:
                        ...
                        if prev_sec == sec:
                            # Même seconde : mise à jour OHLCV en place
                        ...
                        # Nouvelle seconde : finaliser bucket précédent
                        ...
                        # Gap-fill (symboles peu liquides)
                        gap = sec - prev_sec
                        if 1 < gap <= 30:   # max 30s de gap pour éviter les floods
                        ...
                        # Resample immédiat avec verrou pour éviter la concurrence
                ...
                await asyncio.sleep(2)   # backoff réduit (3s → 2s)
...
async def fetch_klines(session: aiohttp.ClientSession, sym: str, interval: str, limit: int) -> pd.DataFrame:
    ...
    for base in (REST_BASE, REST_FALLBACK):
        try:
            async with session.get(f"{base}/api/v3/klines", params=params, timeout=20) as r:
                if r.status != 200:
                    continue
                data = await r.json(content_type=None)
...
_CACHE_REGISTRY: Dict[str, Tuple[Dict, int, int]] = {
    "4h":  (_h4_cache,  H4_CACHE_TTL, H4_LIMIT),
    "2h":  (_h2_cache,  H4_CACHE_TTL, 200),
    "1h":  (_h1_cache,  120,          300),
    "30m": (_m30_cache, 90,           300),
    "15m": (_m15_cache, 60,           300),
    "5m":  (_m5_cache,  M5_CACHE_TTL, M5_LIMIT),
    "3m":  (_m3_cache,  20,           100),
    "1m":  (_m1_cache,  M1_CACHE_TTL, M1_LIMIT),
}
...
def _compute_strict_zones_for_symbol(df: pd.DataFrame, periods: list[pd.DataFrame], progress_hook=None) -> dict:
    ...
    # RELAXED
    floor_relaxed = max(0.3, float(STRICT_SHARPE_FLOOR) - 0.4)
    min_ok_relaxed = max(2, int(STRICT_ROBUST_MIN_PERIODS) - 1)
    ...
    for i in range(iters):
        ...
        for chunk in periods:
            ...
            if m["trades"] < int(STRICT_MIN_TRADES):
                sharpes = []
                break
        ...
        if robust <= -1e8 and robust_rel <= -1e8:
            continue
        ...
        if robust > -1e8 and robust > float(score_best[xi, yj]):
            ...
            best_detail_cell[(xi, yj)] = {
                ...
                "floor": float(STRICT_SHARPE_FLOOR),
                "min_ok": int(STRICT_ROBUST_MIN_PERIODS),
        ...
        if robust_rel > -1e8 and robust_rel > float(score_best_rel[xi, yj]):
            ...
            best_detail_cell_rel[(xi, yj)] = {
                "robust": float(robust_rel),
                "sharpes": [float(s) for s in sharpes],
                "floor": float(floor_relaxed),
                "min_ok": int(min_ok_relaxed),
                "subperiod_days": int(STRICT_SUBPERIOD_DAYS),
            }

    # Choix du mode
    mode = "strict"
    if not best_params_cell:
        if best_params_cell_rel:
            mode = "relaxed"
...
async def strict_recalibrate_symbol(session: aiohttp.ClientSession, sym: str) -> dict:
    ...
    if STRICT_MAX_BARS and len(df) > int(STRICT_MAX_BARS):
        df = df.tail(int(STRICT_MAX_BARS))
    if df.empty or len(df) < 800:
        logger.warning("[STRICT] %s: %s bougies < 800 requises", sym, len(df))
...
async def strict_recalib_loop(session: aiohttp.ClientSession):
    ...
    while True:
        try:
            ...
            df = await fetch_klines_history(session, sym, STRICT_TF, STRICT_IN_SAMPLE_DAYS)
            if df.empty or len(df) < 800:
                logger.warning("[STRICT] Historique insuffisant %s (%s points)", sym, len(df))
                _strict_store[sym] = {"error": "insufficient_history", "next_recalib_ts": now + 3600}
                await asyncio.sleep(10)
                continue
    ...
    while True:
        for sym in SYMBOLS:
            try:
                ...
                df_h4, df_h2, df_h1, df_m30, df_m15, df_1m, df_5m, df_3m = await asyncio.gather(
                ...
                entry, sl, tps = compute_atr_levels(df30, side, df_m3=df_3m, fvgs=fvgs, obs=obs)
                ...
                # ── Module 12 : Alertes Telegram multi-niveaux /7 ─────────
                try:
                    score_int = int(score)
                    if (TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_IDS
                        ...
                        # Zones STRICT robustes
                        strict_z = (strict_info.get("zones") if isinstance(strict_info, dict) else [])
                        z_txt = ""
                        if isinstance(strict_z, list) and strict_z:
                            lines = ["• trix={} signal={} score={:.2f}".format(
                                        z.get('trix_len'), z.get('signal_len'),
                                        float(z.get('score', 0.0) or 0.0))
                                     for z in strict_z[:5] if isinstance(z, dict)]
                            if lines:
                                z_txt = "\n\n<b>ZONES STRICT</b>\n" + "\n".join(lines)

                        # Order Blocks avec statut
                        ob_txt = ""
                        if isinstance(obs, list) and obs:
                            ...
                            for ob in obs[:4]:
                                if not isinstance(ob, dict):
                                    continue
                                status_icon = {"intact": "🟢", "tested": "🟡", "broken": "🔴"}.get(
                                    ob.get("status", "intact"), "⚪")
                                lines.append("• {} {} bot={:.4f} top={:.4f}".format(
                                    status_icon, ob.get("type", "OB"),
                                    float(ob.get("bot", 0) or 0),
                                    float(ob.get("top", 0) or 0)))
                            if lines:
                                ob_txt = "\n\n<b>ORDER BLOCKS</b>\n" + "\n".join(lines)
                        ...
                        if ob_context.get("ob_quality_30m", 0) >= 1.0 or ob_context.get("ob_fvg_15m_ok"):
                            fvg_ob_txt = "\n✅ <b>Double confirmation FVG⊂OB</b>"

                        # Range 30m
                        range_txt = "\n⚠️ <i>Marché en range sur 30m</i>" if ob_context.get("range_30m") else ""
                        ...
                        msg = (
                            "{label}\n"
                            "<b>{sym}</b> — Score <b>{score}/7</b>\n"
                            "Direction: <b>{side}</b>\n"
                            "Confirmations: {confs}\n\n"
                            "Entry (3m): <b>{entry:.6f}</b>\n"
                            "SL: <b>{sl:.6f}</b>\n"
                        ).format(
                            label=quality_label, sym=sym,
                            score=score_int, side=side,
                            confs=" · ".join(confs) if confs else "—",
                        ...
                        if tp3 is not None:
                            msg += "TP3: <b>{:.6f}</b>\n".format(float(tp3))
                        msg += "BE (après TP1): <b>{:.6f}</b>\n".format(float(entry))

                        if isinstance(strict_info, dict) and strict_info.get("sharpe"):
                            msg += "Sharpe TRIX: <b>{:.2f}</b>\n".format(
                                float(strict_info.get('sharpe', 0.0) or 0.0))

                        msg += fvg_ob_txt + range_txt + z_txt + ob_txt
                        msg += "\n\n<i>Vérifiez le dashboard avant exécution.</i>"
...
async def learning_confirm(symbol: str, req: FARequest):
    """Confirme l'adaptation des poids de scoring pour un symbole.

    Body optionnel : {"apply": true} pour appliquer (défaut true).
    Si apply=false : rejette la proposition sans modifier les poids.
    """
    sym = _normalize_sym(symbol)
    if sym not in SYMBOLS:
        raise HTTPException(status_code=404, detail="Symbole inconnu")
    ...
    if not report or report.get("status") != "ready_for_confirm":
        return {"ok": False, "reason": "no_pending_report", "symbol": sym}
    ...
    if apply:
        ...
        return {"ok": True, "applied": True, "symbol": sym, "new_weights": scoring_weights[sym]}
    else:
        _learning_pending_confirm.pop(sym, None)
...
async def vision_analyze(req: FARequest):
    try:
        ...
        if not algo_context and symbol in signals:
            algo_context = {
                "score":      signals[symbol].get("score"),
                "side":       signals[symbol].get("side"),
                "confs":      signals[symbol].get("confs"),
                "entry":      signals[symbol].get("entry"),
                "sl":         signals[symbol].get("sl"),
                "tps":        signals[symbol].get("tps"),
                "supertrend": signals[symbol].get("supertrend"),
                "volume_poc": signals[symbol].get("volume_poc"),
                "patterns":   signals[symbol].get("candle_patterns"),
                "struct_h2":  signals[symbol].get("struct_h2"),
                "struct_h1":  signals[symbol].get("struct_h1"),
                "fvg":        signals[symbol].get("fvg"),
                "ob":         signals[symbol].get("ob"),
            }
...
def main():
    ...
    print("=" * 70)
    print("TITANIUM DASHBOARD v4 — Score /7 · Fib dynamique · Apprentissage · 3m")
    print("URL: http://localhost:8080")
    print("Bridge actif:", ENABLE_BRIDGE)
    print("Vision primary:", VISION_MODEL_PRIMARY, "| fallback:", VISION_MODEL_FALLBACK)
    print(f"SCORING: /7 | RSI seuils: LONG≤{RSI_ENTRY_LONG} SHORT≥{RSI_ENTRY_SHORT} | Fib=[{FIB_LEVEL_LOW},{FIB_LEVEL_HIGH}] dynamique={OB_FVG_FIB_DYNAMIC}")
